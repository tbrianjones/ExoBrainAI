...
**FTS5 improvements.** Three enhancements would improve search quality: (1) Add `tokenize='porter unicode61'` for stemmed search (finding "computing" when searching "compute"); (2) Use `bm25(10.0, 5.0, 1.0)` weighted ranking so title matches rank higher than content matches; (3) Add a composite index `(type_id, created_at DESC)` for the common list-by-type query. These require a future migration (v4+). Note: Migration v3 was used for the projection layer (ADR-007), adding `access_log` table and `projection_override` column.
